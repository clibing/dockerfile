...
docker buildx build --no-cache --platform=${TARGET_PLATFORM_ALL} --build-arg ALPINE_VERSION=${ALPINE_VERSION} -t ${CUSTOM_PREFIX}${IMAGE_BASE_NAME}:${ALPINE_VERSION} --push -f Dockerfile .
